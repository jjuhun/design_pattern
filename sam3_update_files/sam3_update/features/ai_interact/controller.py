...
class AIInteractControllerMixin:
    def _capture_ai_interact_state(self):
        ...
        return {
            ...
            "ai_pending_text_prompt": str(getattr(self, "ai_pending_text_prompt", "") or ""),
        }
    ...
    def _restore_ai_interact_state(self, ai_state):
        ...
        self.ai_pending_text_prompt = str(ai_state.get("ai_pending_text_prompt") or "")
    ...
    def _selected_ai_initial_prompt_mode(self) -> str:
        ...
        if getattr(self, "ai_text_only_prompt_checkbox", None) is not None and self.ai_text_only_prompt_checkbox.isChecked():
            return "text"
    ...
    def on_ai_interact_clicked(self):
        ...
        text_prompt = self._current_sam3_text_prompt() if model_type == "sam3" else ""

        self.ai_interact_pending = True
        self.ai_prompt_mode = prompt_mode
        self.ai_pending_model_type = model_type
        self.ai_pending_text_prompt = text_prompt
        ...
        # SAM3는 text only 모드에서 bbox/pointer 없이 바로 실행한다.
        if prompt_mode == "text":
            if model_type != "sam3":
                self._reset_ai_interact_prompt_state()
                QMessageBox.warning(self, "경고", "text only 모드는 SAM3에서만 사용할 수 있습니다.")
                return
            if not text_prompt:
                self._reset_ai_interact_prompt_state()
                QMessageBox.warning(self, "경고", "SAM3 text only 모드는 text prompt가 필요합니다.")
                return
            if self.ai_interact_button is not None:
        ...
        if prompt_mode == "box":
            self.canvas.set_mode("box")
            if model_type == "sam3" and text_prompt:
                self.show_status_message("SAM3 Interact 대기: 대상 물체를 박스로 드래그하세요. text prompt와 함께 사용됩니다.")
            else:
                self.show_status_message("AI Interact 대기: 대상 물체를 박스로 드래그하세요.")
        else:
            self.canvas.set_mode("ai_point")
            if model_type == "sam3" and text_prompt:
                self.show_status_message("SAM3 Interact 대기: 대상 물체를 점으로 클릭하세요. text prompt 후보 중 점과 맞는 결과를 선택합니다.")
            else:
    ...
    def _current_sam3_text_prompt(self) -> str:
        """오른쪽 SAM3 text prompt 입력값을 안전하게 읽는다."""
        widget = getattr(self, "ai_text_prompt_input", None)
        if widget is None:
            return ""
    ...
    def _execute_ai_single_frame_interact(
        ...
        try:
            if self.ai_pending_model_type == "sam3":
                from features.ai_interact.engine_sam3 import SAM3ImageInteractEngine

                engine = SAM3ImageInteractEngine(
                    device="cuda",
                    polygon_simplification=self._ai_interact_polygon_simplification_value(),
                )
                text_prompt = str(getattr(self, "ai_pending_text_prompt", "") or self._current_sam3_text_prompt()).strip()
                if prompt_box is not None and prompt_points is not None and prompt_labels is not None:
                    mask = engine.segment_with_text_box_and_points(frame, text_prompt, prompt_box, prompt_points, prompt_labels)
                elif prompt_points is not None and prompt_labels is not None:
                    mask = engine.segment_with_text_and_points(frame, text_prompt, prompt_points, prompt_labels)
                elif prompt_box is not None:
                    mask = engine.segment_with_text_and_box(frame, text_prompt, prompt_box)
                elif text_prompt:
                    mask = engine.segment_with_text(frame, text_prompt)
                else:
                    raise RuntimeError("SAM3는 text prompt 또는 bbox/pointer prompt 중 하나가 필요합니다.")
            else:
