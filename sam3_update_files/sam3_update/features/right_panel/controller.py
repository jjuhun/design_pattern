...
class RightPanelControllerMixin:
    def build_right_panel(self):
        ...
        self.ai_text_only_prompt_checkbox = QCheckBox("text only")
        ...
        self.ai_prompt_mode_button_group.addButton(self.ai_text_only_prompt_checkbox)
        prompt_input_layout.addWidget(self.ai_bbox_prompt_checkbox)
        prompt_input_layout.addWidget(self.ai_pointer_prompt_checkbox)
        prompt_input_layout.addWidget(self.ai_text_only_prompt_checkbox)
    ...
    def on_ai_interactor_changed(self, index):
        ...
        is_sam3 = model_type == "sam3"
        self.ai_text_prompt_container.setVisible(is_sam3)
        if getattr(self, "ai_text_only_prompt_checkbox", None) is not None:
            self.ai_text_only_prompt_checkbox.setVisible(is_sam3)
            self.ai_text_only_prompt_checkbox.setEnabled(is_sam3)
            if not is_sam3 and self.ai_text_only_prompt_checkbox.isChecked():
                self.ai_bbox_prompt_checkbox.setChecked(True)
